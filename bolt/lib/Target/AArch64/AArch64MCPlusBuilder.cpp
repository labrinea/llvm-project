...
public:
  ...
  bool isReversibleBranch(
      const MCInst &Inst,
      const BranchLivenessInfo *BranchLiveness = nullptr) const override {
    if (isCompAndBranch(Inst)) {
      bool MustPreserveFlags =
          BranchLiveness ? BranchLiveness->mustPreserveFlags(Inst) : true;
  ...
  void reverseBranchCondition(
      ...
      MCContext *Ctx,
      const BranchLivenessInfo *BranchLiveness = nullptr) const override {
    assert(isReversibleBranch(Inst, BranchLiveness) && "Irreversible branch");
