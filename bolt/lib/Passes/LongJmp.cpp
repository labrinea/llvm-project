...
void LongJmpPass::relaxCalls(BinaryContext &BC,
  ...
  auto getDirectCallTarget = [&](const BinaryBasicBlock &BB,
                                 const MCInst &Inst) -> const MCSymbol * {
    ...
    if (BC.MIB->isUnconditionalBranch(Inst) && BB.getSuccessor(TargetSymbol))
      return nullptr;
  ...
  for (BinaryFunction *BF : OutputFunctions) {
    if (!BC.shouldEmit(*BF) || BF->isPatch() || !BF->isSimple())
      continue;

    for (BinaryBasicBlock &BB : *BF) {
      const unsigned SourceCluster = BBToCluster[&BB];

      for (MCInst &Inst : BB) {
        const MCSymbol *TargetSymbol = getDirectCallTarget(BB, Inst);
        if (!TargetSymbol)
          continue;

        // If not found TargetCluster becomes UINT_MAX.
        auto It = SymToCluster.find(TargetSymbol);
        unsigned TargetCluster = It == SymToCluster.end() ? -1 : It->second;
...
void LongJmpPass::relaxUnconditionalBranches(
  ...
  for (BinaryFunction *BF : OutputFunctions) {
    ...
    for (BinaryBasicBlock &BB : *BF) {
      ...
      for (MCInst &Inst : BB) {
        ...
        const unsigned TargetCluster = BBToCluster[TargetBB];
        if (SourceCluster == TargetCluster)
          continue;

        CrossClusterBranches.push_back(
            {&Inst, TargetBB, SourceCluster, TargetCluster});
      }
    }
  }
  ...
  auto getOrCreateBranchThunk =
      [&](FragmentCluster &Cluster, const BinaryBasicBlock *TargetBB,
  ...
  auto getOrCreateBranchThunkChain =
    ...
    const MCSymbol *NextTarget = TargetBB.getLabel();

    if (SourceCluster < TargetCluster) {
      for (unsigned Cluster = TargetCluster; Cluster > SourceCluster;) {
        --Cluster;
        FirstThunk = getOrCreateBranchThunk(Clusters[Cluster], &TargetBB,
